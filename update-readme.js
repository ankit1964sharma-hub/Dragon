...
async function push() {
  ...
  const { data: commit } = await octokit.repos.getCommit({
    owner: 'ankit1964sharma-hub', repo: 'Dragon', ref: 'main'
  });
  ...
  const { data: newCommit } = await octokit.git.createCommit({
    owner: 'ankit1964sharma-hub', repo: 'Dragon',
    message: 'Update documentation: 100% JavaScript with dynamic asset generation',
    tree: tree.sha, parents: [commit.sha]
  });
